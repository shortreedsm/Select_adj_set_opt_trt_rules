...
CBS <- function(X,D,Y,alpha = 0.05,t_ind){
  p = ncol(X)
  n = nrow(X)
  threshold = min(30,p) 
  ...
  #NINA EDIT, add in tailoring index
  if (t_ind %in% ballsisselectedindex){
    # the data matrix after screening
    Data = X[,ballsisselectedindex]
    
    weight[which(ballsisselectedindex == t_ind)] = Inf
    
    lasso_inds <- ballsisselectedindex
    
  } else{
    
    Data = X[,c(ballsisselectedindex,t_ind)]
    
    weight = c(weight,Inf)
    
    var.list_Ball <- c(var.list_Ball,"tailoring")
    
    lasso_inds <- c(ballsisselectedindex, t_ind)
  ...
  betaXY = weight
  #NINA EDIT IGNORE TAILORING WEIGHT
  betaXY = weight/max(weight[weight < Inf])
  ...
  for( lil in names(lambda_val) )
    for(mim in names(gamma_vals)){
      ...
      #NINA EDIT
      fitx = as.matrix(Data[,var.list_Ball])
      ...
      #NINA edit save model
      mod_list[[paste0( lil,mim)]] <- alasso
  ...
  final_mod <- mod_list[[tt]]
  
  #NINA EDIT get selected vars
  final_coefs <-  coeff_XA[var.list_Ball,names(tt)]
  
  selected <- rep(0, p)

  selected_lasso <- lasso_inds[abs(final_coefs) > 10^(-5)]
  
  selected[selected_lasso] <- 1

 
  #save the estimated propensity score model
  return(list(fitted.ps = Data[,paste("f.pA",names(tt),sep="")],
              selected = selected,
              final_mod = final_mod,
              lasso_inds = lasso_inds))
